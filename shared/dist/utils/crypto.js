...
export async function signDocument(content, privateKey) {
    const encoder = new TextEncoder();
    const data = encoder.encode(content);
...
export async function verifySignature(content, signature, publicKey) {
    try {
        const encoder = new TextEncoder();
        const data = encoder.encode(content);
...
export function isSignatureExpired(signature, expirationHours) {
    const now = Date.now();
